...
function generateCompletionEmailHtml(order: OrderPayload, brand: Brand, logoUrl: string | null): string {
  const { orderInfo, items } = order
...
`,
  ...
  <div style="font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: auto; border: 1px solid #ddd; padding: 20px;">
    ...
    <p>We're pleased to inform you that your printing order (<strong>#${orderInfo.orderNumber}</strong>) for <strong>${brand.name}</strong> has been completed by the printer and is on its way.</p>
    
    <h3 style="color: ${themeColor}; font-size: 20px; border-bottom: 2px solid ${themeColor}; padding-bottom: 5px;">Order Summary</h3>
    <table style="width: 100%; border-collapse: collapse; margin-top: 10px;">
      <thead>
        <tr>
          <th style="padding: 10px; text-align: left; background-color: #f7f7f7;">Code</th>
...
export async function sendOrderEmail(order: OrderPayload, brand: Brand, pdfBuffer: Buffer, logoUrl: string | null) {
  ...
  // Add the person who ordered to the CC list
...
export async function sendCompletionEmail(order: OrderPayload, brand: Brand, logoUrl: string | null) {
  const transporter = nodemailer.createTransport({
    host: "smtp.mailgun.org",
    port: 587,
    secure: false,
  ...
  const mailOptions = {
    from: `"${brand.name} Orders" <${process.env.FROM_EMAIL}>`,
    to: order.orderInfo.email, // Send to the user who ordered
    subject: `Your Printing Order #${order.orderInfo.orderNumber} is Complete`,
    html: generateCompletionEmailHtml(order, brand, logoUrl),
  }
