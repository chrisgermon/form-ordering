...
export function BrandForm({ brand, uploadedFiles, onSave, onCancel, onLogoUpload }: BrandFormProps) {
  ...
  const logoSelectionFiles = useMemo(() => {
    // Only show image files in the logo selector
    return uploadedFiles.filter((file) => file.content_type?.startsWith("image/"))
  }, [uploadedFiles])
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmitHandler)} className="flex flex-col h-full overflow-hidden">
        <div className="flex-grow overflow-y-auto pr-6 -mr-6 space-y-6 mt-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Upload logos and other images in the global 'Files' tab on the main dashboard.
            </p>
          </div>
