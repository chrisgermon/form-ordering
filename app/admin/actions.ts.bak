...
export async function fetchClinicLocationsFromUrl(url: string) {
  ...
  try {
    ...
    $("script, style, header, footer, nav, svg, noscript, iframe, link, meta").remove()
        ...
        locations: z
          .array(
            z.object({
              ...
              phone: z.string().optional().describe("The primary contact phone number for the clinic."),
            }),
          )
    ...
    if (!locationsData || !Array.isArray(locationsData.locations) || locationsData.locations.length === 0) {
      return { success: false, message: "The AI could not find any clinic locations on the provided website." }
    }
